...
def playlistRecommender():
    ...
    if access_token:
        ...
        recommended_tracks = sp.recommendations(limit=5,seed_artists=["4NHQUGzhtTLFvgF5SZesLK"],seed_genres=["country"], seed_tracks=["0c6xIDDpzE81m2q797ordA"])
        tracks_id = []
        ...
        init_playlist = sp.user_playlist_create(user_id, "NextSong", public=True, collaborative=False, description='Your curated ML music recommender! by NextSong')
        playlist_embed_link=init_playlist["external_urls"]["spotify"]
        user_playlist_id= init_playlist["id"]
        creating_playlist=sp.user_playlist_add_tracks(user_id,user_playlist_id,tracks_id, position=None)
